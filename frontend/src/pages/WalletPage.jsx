...
export default function WalletPage() {
  ...
  async function handleEmergencyRequest(e) {
    ...
    if (!amt || amt < 10) {
      setEmergencyError(t('minEmergency') || 'Minimum ৳10')
      return
    }
    if (amt > emergencyStatus.available) {
      setEmergencyError(
        `${t('maxEmergency') || 'Maximum available'}: ৳${emergencyStatus.available.toFixed(2)}`
      )
      return
    }
  ...
  return (
    <div className={styles.page}>
          ...
          <div className={styles.txList}>
            {transactions.map((tx, i) => (
              <div key={i} className={styles.txRow} style={{ animationDelay: `${i * 30}ms` }}>
                <div
                  className={`${styles.txBadge} ${tx.type === 'credit' ? styles.txBadgeCredit : styles.txBadgeDebit}`}
                >
                  {tx.type === 'credit' ? (
                    <svg
                      width="14"
                ...
                <div className={styles.txRight}>
                  <span
                    className={`${styles.txAmount} ${tx.type === 'credit' ? styles.txAmtCredit : styles.txAmtDebit}`}
                  >
      ...
      {/* Top-up modal */}
      {showTopUp && (
        <WalletTopUp onClose={() => setShowTopUp(false)} onSuccess={handleTopUpSuccess} />
      )}
        ...
        >
          <div className={styles.emergencyModal}>
            <div className={styles.emergencyHeader}>
              ...
              <button className={styles.emergencyClose} onClick={() => setShowEmergency(false)}>
                ✕
              </button>
            </div>
            ...
            <div className={styles.emergencyInfo}>
              ...
              <p>
                {t('emergencyDesc') ||
                  'Take an advance from your IUT monthly allowance. This amount will be automatically deducted from your next bank allowance.'}
              </p>
            </div>
            ...
            <div className={styles.emergencyStats}>
              <div className={styles.emergencyStat}>
                ...
                <span className={styles.emergencyStatVal}>
                  ৳{emergencyStatus.limit?.toFixed(2)}
                </span>
              </div>
              <div className={styles.emergencyStat}>
                <span className={styles.emergencyStatLabel}>
                  {t('emergencyOutstanding') || 'Outstanding'}
                </span>
                <span className={styles.emergencyStatValRed}>
                  ৳{emergencyStatus.outstanding?.toFixed(2)}
                </span>
              </div>
              <div className={styles.emergencyStat}>
                <span className={styles.emergencyStatLabel}>
                  {t('emergencyAvailable') || 'Available'}
                </span>
                <span className={styles.emergencyStatValGreen}>
                  ৳{emergencyStatus.available?.toFixed(2)}
                </span>
              </div>
            </div>
            ...
            <form onSubmit={handleEmergencyRequest} className={styles.emergencyForm}>
              <div className={styles.emergencyField}>
                ...
                <div className={styles.emergencyQuickAmounts}>
                  {[100, 200, 500, 1000]
                    .filter(a => a <= emergencyStatus.available)
                    .map(a => (
                      <button
                        key={a}
                        type="button"
                        className={styles.emergencyQuickBtn}
                        onClick={() => setEmergencyAmount(String(a))}
                      >
                        ৳{a}
                      </button>
                    ))}
              ...
              >
                {emergencyLoading ? (
                  ...
                  `${t('emergencyTake') || 'Take'} ৳${emergencyAmount || '0'}`
                )}
              </button>

              <p className={styles.emergencyNote}>
                {t('emergencyNote') ||
                  '⚠️ This amount will be deducted from your next IUT monthly allowance.'}
              </p>
            </form>
          </div>
        </div>
