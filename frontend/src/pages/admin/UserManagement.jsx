...
export default function UserManagement() {
  ...
  async function toggleUserStatus(user) {
    ...
    try {
      await axios.put(`/admin/identity/admin/users/${user.student_id}/status`, {
        is_active: newIsActive,
      })
      setUsers(
        users.map(u => (u.student_id === user.student_id ? { ...u, is_active: newIsActive } : u))
      )
      if (selectedUser?.student_id === user.student_id) {
        setUserActivity(prev =>
          prev ? { ...prev, user: { ...prev.user, is_active: newIsActive } } : null
        )
      }
    } catch (err) {
          ...
          <tbody>
            {users.map(user => (
              <tr key={user.student_id}>
                ...
                <td>
                  <span
                    className={`${styles.badge} ${user.is_active ? styles.badgeSuccess : styles.badgeDanger}`}
                  >
              ...
              {activityLoading ? (
                ...
                <div className={styles.activityContent}>
                  <div className={styles.userInfo}>
                    <p>
                      <strong>Student ID:</strong> {userActivity.user?.student_id}
                    </p>
                    <p>
                      <strong>Name:</strong> {userActivity.user?.name}
                    </p>
                    <p>
                      <strong>Department:</strong> {userActivity.user?.department}
                    </p>
                    <p>
                      <strong>Status:</strong>
                      <span
                        className={`${styles.badge} ${userActivity.user?.is_active ? styles.badgeSuccess : styles.badgeDanger}`}
                    ...
                    <p>
                      ...
                      {userActivity.user?.created_at
                        ? new Date(userActivity.user.created_at).toLocaleDateString()
                        : '—'}
                    </p>
                  </div>
                  ...
                  <div className={styles.modalActions}>
                    <button
                      className={
                        userActivity.user?.is_active ? styles.btnDanger : styles.btnSuccess
                      }
