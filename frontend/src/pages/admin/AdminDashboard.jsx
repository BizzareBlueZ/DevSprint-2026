    ...
    <div className={styles.page}>
      ...
      <div className={styles.topBar}>
        <div>
          ...
          <div className={styles.pageSub}>
            <span
              className={`${styles.pulse} ${onlineCount === SERVICES.length ? styles.pulseGreen : styles.pulseAmber}`}
            />
      ...
      <div className={styles.statsRow}>
        <StatCard
          ...
          accent={onlineCount === SERVICES.length ? '#10b981' : '#f59e0b'}
        />
        ...
        <StatCard
          ...
          label="Failed Orders"
          value={totalFails}
          accent={totalFails > 0 ? '#ef4444' : '#6b7a8d'}
        />
        <StatCard
          icon="⚡"
          label="Avg Latency"
          value={`${avgLatency}ms`}
          accent={avgLatency > 1000 ? '#ef4444' : avgLatency > 500 ? '#f59e0b' : '#10b981'}
        />
      </div>
        ...
        <div className={styles.sectionHead}>
          ...
          <div className={styles.sectionBtns}>
            <button
              ...
              onClick={() => SERVICES.forEach(s => !killed[s.id] && toggleChaos(s))}
            >
              ⏹ Kill All
            </button>
            <button
        ...
        <div className={styles.healthGrid}>
          {SERVICES.map(svc => {
            ...
            return (
              <div
                key={svc.id}
                className={`${styles.hCard} ${isOnline ? styles.hCardOnline : styles.hCardDown}`}
              >
                <div
                  className={`${styles.hCardStripe} ${isOnline ? styles.stripeGreen : styles.stripeRed}`}
                />

                <div className={styles.hCardTop}>
                  <span className={styles.hIcon}>{svc.icon}</span>
                  <span
                    className={`${styles.hBadge} ${isKilled ? styles.badgeKilled : isOnline ? styles.badgeOnline : styles.badgeDown}`}
                  >
                ...
                <div className={styles.hMetrics}>
                  ...
                  <div className={styles.hMetric}>
                    <span
                      className={`${styles.hMetricVal} ${lat > 1000 ? styles.valRed : lat > 500 ? styles.valAmber : lat != null ? styles.valGreen : ''}`}
                    >
                      {lat != null ? `${lat}ms` : '—'}
                    </span>
          ...
          {filteredServices.map(svc => {
            ...
            return (
              <div key={svc.id} className={`${styles.tRow} ${!isOnline ? styles.tRowDim : ''}`}>
                ...
                <div>
                  <span
                    className={`${styles.pill} ${isKilled ? styles.pillKilled : isOnline ? styles.pillOnline : styles.pillDown}`}
                  >
                ...
                <div
                  className={`${styles.tNum} ${(m?.failureCount || 0) > 0 ? styles.numRed : styles.numMuted}`}
                >
                  {m?.failureCount ?? '—'}
                </div>
                <div
                  className={`${styles.tNum} ${(m?.averageLatencyMs || 0) > 1000 ? styles.numRed : (m?.averageLatencyMs || 0) > 500 ? styles.numAmber : styles.numGreen}`}
                >
        ...
        <div className={styles.chaosPanel}>
          ...
          <div className={styles.chaosPanelBody}>
            ...
            <p className={styles.chaosPanelDesc}>
              ...
              Killing <strong>Notification Hub</strong> should not block order placement. Killing{' '}
              <strong>Kitchen Queue</strong> pauses cooking but preserves orders in RabbitMQ. Use{' '}
              <strong>Restore All</strong> to recover the system.
            </p>
          </div>
          <div className={styles.chaosPanelStats}>
            <div className={styles.cStat}>
              <span className={styles.cStatNum} style={{ color: '#10b981' }}>
                {onlineCount}
              </span>
              <span className={styles.cStatLbl}>Online</span>
            </div>
            <div className={styles.cDivider} />
            <div className={styles.cStat}>
              <span className={styles.cStatNum} style={{ color: '#ef4444' }}>
                {SERVICES.length - onlineCount}
              </span>
              <span className={styles.cStatLbl}>Killed</span>
            </div>
          </div>
        </div>
      </section>
