...
export default function RegisterPage() {
  ...
  const pwRules = [
    ...
    {
      test: /[^a-zA-Z0-9\s]/.test(form.password),
  ...
  async function handleSubmit(e) {
    ...
    setError('')
    if (!pwValid) {
      setError(t('passwordStrength') + ' requirement not met.')
      return
    }
    setLoading(true)
    try {
      ...
      <div className={styles.card}>
        <div className={styles.cardHeader}>
          ...
          <div>
            ...
            <p className={styles.sub}>{t('registerWithIUT')}</p>
          </div>
        </div>
        <form onSubmit={handleSubmit} className={styles.form}>
          <div className={styles.field}>
            <label className={styles.label}>{t('studentId')}</label>
            <input
              className={styles.input}
              placeholder="e.g. 230042135"
            ...
            <span className={styles.hint}>
              {form.studentId && `→ ${form.studentId}@iut-dhaka.edu`}
            </span>
          </div>
          <div className={styles.field}>
          ...
          <button type="submit" className={styles.btn} disabled={loading}>
            {loading ? (
              ...
              <>
                {t('createAccount')} <span className={styles.arrow}>→</span>
              </>
            )}
          </button>
        </form>
        <p className={styles.loginLink}>
          {t('alreadyHave')} <Link to="/login">{t('signIn')}</Link>
        </p>
      </div>
    </div>
  )
