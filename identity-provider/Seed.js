...
if (!process.env.SEED_ADMIN_PASSWORD || !process.env.SEED_IUTCS_ADMIN_PASSWORD) {
  ...
  console.warn(
    '   Using insecure placeholder passwords. Set these before running in any shared environment!'
  )
}
...
const ADMINS = [
  ...
  {
    username: 'iutcs',
    email: 'cs@iut-dhaka.edu',
    fullName: 'CS Department Admin',
    password: SEED_IUTCS_ADMIN_PASSWORD,
    role: 'admin',
  },
]
...
async function seed() {
  ...
  for (const a of ADMINS) {
    ...
    await pool.query(
      `INSERT INTO identity.admins (username, email, password_hash, full_name, role)
             VALUES ($1, $2, $3, $4, $5)
  ...
  console.log(
    `📋 Admin login:           admin / [SEED_ADMIN_PASSWORD]  OR  iutcs / [SEED_IUTCS_ADMIN_PASSWORD]`
  )
  await pool.end()
}
