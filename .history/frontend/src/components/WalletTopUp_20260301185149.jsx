...
export default function WalletTopUp({ onClose, onSuccess }) {
  ...
  async function handleProceed(e) {
    ...
    if (method !== 'bank' && !phone) { setError('Please enter your mobile number.'); return }
    // Simulate OTP step
    setStep('otp')
  }
  ...
  async function handleConfirm(e) {
    e.preventDefault()
    setError('')
    if (otp.length < 4) { setError('Enter the 6-digit OTP sent to your number.'); return }
    setLoading(true)
    try {
      // Call backend to credit the wallet
