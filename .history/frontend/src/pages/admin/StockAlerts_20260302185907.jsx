    ...
    return (
        <div className={styles.page}>
            ...
            <div className={styles.alertSummary}>
                <div className={`${styles.alertCard} ${alerts.length > 0 ? styles.alertDanger : styles.alertSuccess}`}>
                        ...
                        {displayItems.map(item => (
                            <tr key={item.item_id} className={item.low_stock ? styles.lowStockRow : ''}>
                                ...
                                <td>
                                    {editingItem === item.item_id ? (
                                        <input
                                            ...
                                            onChange={e => setEditValues({ ...editValues, threshold: parseInt(e.target.value) || 10 })}
                                            className={styles.inlineInput}
                                        />
                                    ) : (
                                        item.threshold || 10
                                ...
                                <td className={styles.actions}>
                                    {editingItem === item.item_id ? (
                                        <>
                                            <button className={styles.btnSuccess} onClick={() => saveEdit(item.item_id)}>Save</button>
                                            <button className={styles.btnSecondary} onClick={cancelEdit}>Cancel</button>
                                        </>
                                    ) : (
                                        <button className={styles.btnEdit} onClick={() => startEdit(item)}>✏️ Update</button>
                                    )}
                                </td>
                            </tr>
                        ))}
            ...
            <div className={styles.infoPanel}>
                ...
                <p>Items are flagged as "Low Stock" when their quantity drops below the threshold (default: 10).</p>
