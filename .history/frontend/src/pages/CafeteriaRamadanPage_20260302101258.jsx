...
export default function CafeteriaRamadanPage() {
  ...
  function isDateBooked(date) {
    return bookedTokens.some(t => t.type === 'iftar' && isSameDay(parseISO(t.date), date))
  }
  ...
  function toggleDate(date) {
    if (!isDateInRange(date) || isDateBooked(date)) return
  ...
  const totalBookingCost = selectedDates.length * IFTAR_TOKEN_PRICE
    ...
    <div className={styles.page}>
      ...
      <div className={styles.advanceSection}>
        <div className={styles.advanceTitleRow}>
          <div>
            <div className={styles.advanceTitle}>Book Iftar in Advance</div>
            <div className={styles.advanceSubtitle}>
              Select upcoming dates to pre-book your iftar meals (৳{IFTAR_TOKEN_PRICE}/day)
            </div>
          </div>
        ...
        {bookedTokens.filter(t => t.type === 'iftar').length > 0 && (
          <div className={styles.bookedTokensRow}>
            <span className={styles.bookedLabel}>Already booked:</span>
            {bookedTokens.filter(t => t.type === 'iftar').map((t, i) => (
        ...
        {selectedDates.length > 0 && (
          <div className={styles.bookingSummary}>
            ...
            >
              {bookingLoading ? 'Booking...' : totalBookingCost > balance ? 'Insufficient Balance' : 'Confirm Advance Booking'}
            </button>
          </div>
        )}
